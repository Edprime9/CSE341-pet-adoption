...
const petsController = {
    ...
    getSpecificPet: async (req, res) => {
        try {
            const pet = await petsModel.getCollection('pets').findOne({ _id: new _id(req.params.id) });
    ...
    postNewPet: async (req, res) => {
        try {
            const { name, species, breed, age, gender, size, status, shelter_id, description, traits, intake_date } = req.body;
            ...
            res.status(201).json({ message: 'New pet added successfully', petId: addPet.insertedId });
        }
        catch (error) {
    ...
    editPet: async (req, res) => {
        try {
            const { name, species, breed, age, gender, size, status, shelter_id, description, traits, intake_date } = req.body;
            const editPet = await petsModel.getCollection('pets').updateOne({ id: new _id(req.params.id) }, { $set: { name, species, breed, age, gender, size, status, shelter_id, description, traits, intake_date } });
    ...
    deletePet: async (req, res) => {
        try {
            const deletePet = petsModel.getCollection('pets').deleteOne({ id: new _id(req.params.id) });
